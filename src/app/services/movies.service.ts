...
export class MoviesService {
  private apiUrl = 'https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=1';
  ...
  getMoviesByGenre(genre: string): Observable<any> {
    return this.http.get(`${this.apiUrl}&with_genres=${genre}`, { headers: this.headers }).pipe(
      tap((data: any) => {
        localStorage.setItem('movies', JSON.stringify(data));
      }),
      catchError(error => {
